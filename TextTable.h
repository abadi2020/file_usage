...
public:
	...
	/*void add(int const & content)
	{
		_current.push_back(std::to_string(content));
	}
	*/
...
{int first = 0;
	...
	for (auto rowIterator = table.rows().begin(); rowIterator != table.rows().end(); ++rowIterator) {
		...
		for (unsigned i = 0; i < row.size(); ++i) {
			auto alignment = table.alignment(i) == TextTable::Alignment::RIGHT ? std::right : std::right;
			stream << std::setw(table.width(i)) << alignment << row[i];
			stream << ' '; stream << table.vertical(); stream << ' ';
		}
		stream << "\n";
		first++;
		if (first == 1)
		...
		else if(rowIterator == table.rows().end() - 2)
			stream << table.ruler() << "\n";
	}
